namespace Binance.Common.Tests
{
    using Xunit;

    public class BinanceRsa_Tests
    {
        #region Sign
        [Fact]
        public void Sign_Unprotected_Payload()
        {
            var pem = @"-----BEGIN PRIVATE KEY-----

miJlY6JEKpSxvkrS04XrbvHk
-----END PRIVATE KEY-----";
            var payload = "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559";
            var expectedSignature = "";

            var signatureProvider = new BinanceRsa(pem);

            var signature = signatureProvider.Sign(payload);

            Assert.Equal(expectedSignature, signature);
        }

        [Fact]
        public void Sign_Protected_Payload()
        {
            var pem = @"-----BEGIN ENCRYPTED PRIVATE KEY-----

-----END ENCRYPTED PRIVATE KEY-----";
            var password = "password";
            var payload = "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559";
            var expectedSignature = "";

            var signatureProvider = new BinanceRsa(pem, password);

            var signature = signatureProvider.Sign(payload);

            Assert.Equal(expectedSignature, signature);
        }
        #endregion
    }
}